...
int main() {
    ...
    imprimirLista("Insertion Sort | Melhor Caso (já ordenada)", listaTemp, TAMANHO);
    ...
    copiarLista(listaTemp, listaAleatoria, TAMANHO);
    insertionSort(listaTemp, TAMANHO);
    imprimirLista("Insertion Sort | Caso Médio (aleatória)", listaTemp, TAMANHO);
...
void bubbleSort(int lista[], int n) {
    ...
    for (i = 0; i < n - 1; i++) {
        trocou = 0; // No início de cada passagem, assumimos que não haverá trocas.
        ...
        for (j = 0; j < n - i - 1; j++) {
            ...
            if (lista[j] > lista[j + 1]) {
                ...
                lista[j] = lista[j + 1];
                lista[j + 1] = temp;
                trocou = 1; //Marcar que uma troca ocorreu.
            }
        }
...
void insertionSort(int lista[], int n) {
    ...
    // Começa do segundo elemento (índice 1), pois o primeiro já é uma sub-lista ordenada.
    for (i = 1; i < n; i++) {
...
void gerarListaAleatoria(int lista[], int n) {
    ...
    imprimirLista("Original - Aleatória", lista, n);
}

/**
 * @brief Imprime uma lista de inteiros com um título descritivo.
 * @param titulo Texto que identifica o contexto da lista (ex: "Bubble Sort | Melhor Caso").
 * @param lista Vetor de inteiros a ser impresso.
 ...
 * @param destino Vetor que receberá os dados copiados.
 * @param origem Vetor de onde os dados serão copiados.
 * @param n Tamanho das listas.
 */
void copiarLista(int destino[], const int origem[], int n) {
    for (int i = 0; i < n; i++) {
        destino[i] = origem[i];
